...
Environment variables (create a .env file):
    ...
    SMTP_FROM=FINDME <you@gmail.com>
...
# IMPORTS
# ─────────────────────────────────────────────────────────────────────────────
import os, io, uuid, json, logging, smtplib, re
...
def make_token(user_id: str) -> str:
    expire = datetime.utcnow() + timedelta(minutes=cfg().ACCESS_TOKEN_EXPIRE_MINS)
...
def _send_sms(to: str, body: str):
    s = cfg()
    if not s.TWILIO_ACCOUNT_SID: return
...
async def notify_match(user: dict, lost: dict, found: dict, match: dict, finder: dict = None):
    ...
    if user.get("phone"):
        sms = (f"FINDME: {int(match.get('score',0))}% match for "
               f"'{lost.get('name','')}'. Found: '{found.get('name','')}' "
               f"at {found.get('location','campus office')}. Log in to confirm.")
        st, err = "sent", None
        try:    _send_sms(user["phone"], sms)
        except Exception as e: st, err = "failed", str(e); logger.warning(f"SMS: {e}")
        await col.insert_one({"user_id":uid,"match_id":mid,"type":"sms","status":st,
                              "recipient":user["phone"],"subject":None,"body":sms,
                              "sent_at":now if st=="sent" else None,"error_message":err,"created_at":now})
